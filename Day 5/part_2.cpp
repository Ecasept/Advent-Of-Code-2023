...
long findMinCalc(const std::vector<Range> &seeds,
                 const std::vector<std::vector<std::vector<long>>> &maps) {
    long minResult = std::numeric_limits<long>::max();
    long i = 0;
    for (const auto &seed : seeds) {
        Range mutableSeed = seed;
        long result = mutableSeed.next(); // Get the first value from the Range
        while (result != -1) {
            long calcResult = calc(result, maps);
            minResult = std::min(minResult, calcResult);
            result = mutableSeed.next(); // Get the next value from the Range
