...
class Hand:
    ...
    def get_type_value_with_jokers(self) -> str:
        highest_value = 0
        for possibility in ["A", "K", "Q", "J", "T", "9", "8", "7", "6", "5", "4", "3", "2"]:
            val = self.get_type_value(self.cards.replace("J", possibility))
            val = int(val)
    ...
    def get_type_value(self, cards) -> str:
        counts = []
        while cards:
            symbol = cards[0]
    ...
    def get_sorting_value(self) -> str:
        return self.get_type_value_with_jokers() + self.get_cards_value()
hands: list[Hand] = []
for line in data:
    line = line.strip()
